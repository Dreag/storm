package L18bigdata.storm.trident.state.election;
...
public class TridentStateExample {
	...
	public static class RandomVoteSpout implements IBatchSpout {
		...
		public void emitBatch(long batchId, TridentCollector collector) {
			...
			for (Integer count = 0; count < 100; count++) {
				...
				collector
						.emit(new Values(this._constituencyId, partyNames[_rand
								.nextInt(partyNames.length)].toString()));
			}
		}
	...
	public static class VoteCountAggregator extends
			BaseAggregator<VoteCountAggregator.CountState> {
		private static final Logger LOG = Logger
				.getLogger(VoteCountAggregator.class);
		...
		public void aggregate(CountState state, TridentTuple tuple,
			...
			PartyName partyName = PartyName.valueOf(tuple
					.getStringByField("PartyName"));
			LOG.debug(String.format("Got tuple %s %s %s", constituencyId,
					partyName, state.state.DemocratsVotes));
		...
		public void complete(CountState state, TridentCollector collector) {
			LOG.debug(String.format("%s Emitting counts "
					+ "for Constituency %d" + " - Democracts %d "
					+ "Republican %d for Batch %s", hashCode(),
					state.constituencyId, state.state.DemocratsVotes,
					state.state.RepublicanVotes, _batchId));
	...
	public static class StateUpdater extends BaseStateUpdater<ElectionState> {
		public void updateState(ElectionState state, List<TridentTuple> tuples,
				TridentCollector collector) {
	...
	public static class StateUpdaterFactory implements StateFactory {
		...
		public State makeState(Map conf, IMetricsContext metricsContext,
	...
	public static class ElectionState implements State {
		...
		public void setStateValues(List<Integer> constituencyList,
			...
			for (int i = 0; i < constituencyList.size(); i++) {
				...
				ConstituencyState batchUpdateForConstituency = batchStates
						.get(i);
				if (!totalCount.containsKey(constituencyId)) {
					totalCount.put(constituencyId, new ConstituencyState());
				}

				ConstituencyState totalCountsForConstituency = totalCount
						.get(constituencyId);
				...
				LOG.debug(String.format("%s - Commited counts for "
						...
						+ "Democrats %d Republican %d ", hashCode(),
						constituencyId,
		...
		public List<ConstituencyState> getCurrentCountsForConstituency(
				List<Integer> constituencyList) {
			List<ConstituencyState> states = new ArrayList<ConstituencyState>();
			for (int i = 0; i < constituencyList.size(); i++) {
				Integer constituencyId = constituencyList.get(i);
				LOG.debug(String.format("GET for %d %d %s", constituencyId,
						totalCount.size(), hashCode()));
				if (totalCount.containsKey(constituencyId)) {
	...
	public static class QueryConstituencyCount extends
			BaseQueryFunction<ElectionState, Object> {
		
		public List<Object> batchRetrieve(ElectionState state,
			...
			List<ConstituencyState> currentStates = state
					.getCurrentCountsForConstituency(ids);
	...
	public static class QueryElectionLeader extends
			BaseQueryFunction<ElectionState, Object> {
		public List<Object> batchRetrieve(ElectionState state,
				List<TridentTuple> inputs) {
			List<Integer> ids = new ArrayList<Integer>();
	...
	public static void main(String[] args) {
		...
		TridentState loginCounts = topology
				.newStream("voting-events", new RandomVoteSpout())
		...
		topology.newDRPCStream("vote-count", drpc)
				.stateQuery(loginCounts, new Fields("args"),
						new QueryConstituencyCount(), new Fields("count"))
				.each(new Fields("count"), new FilterNull());

		topology.newDRPCStream("leader", drpc)
				.stateQuery(loginCounts, new Fields("args"),
						new QueryElectionLeader(), new Fields("count"))
				.each(new Fields("count"), new FilterNull());
		...
		for (int i = 0; i < 1000; i++) {
			System.out.println(String.format(
					"Current Counts for Constituency %s",
					drpc.execute("vote-count", "1")));
			System.out.println(String.format("Current Election Counts %s",
					drpc.execute("leader", "")));
			Utils.sleep(2000);
		}
		cluster.shutdown();
